...
namespace CRL.Account
{
    ...
    /// 此逻辑会生成详细的帐户变动情况
    /// </summary>
    public class TransactionAction<TType> : BaseAction<TType, ITransaction> where TType : class
    {
        public static new string CreateTable()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetSerialNumber(int transactionType, object tradeType, int operateType)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static ITransaction GetTransaction(int id)
        {
            ITransaction item = QueryItem<ITransaction>(id);
            return item;
        }
        ...
        /// <returns></returns>
        public static bool CheckTransactionSubmited(DBExtend helper,ITransaction item)
        {
        ...
        /// <returns></returns>
        public static bool SubmitTransaction(out string error,params ITransaction[] items)
        {
            error = "";
            if (items.Length == 0)
            {
        ...
        /// <param name="error"></param>
        /// <returns></returns>
        public static bool SubmitTransaction(DBExtend helper, ITransaction item,out string error)
        {
            ...
            //}
            error = "";
            var account = AccountAction<TType>.GetAccountFromCache(item.AccountId);
...
update $IAccountDetail set CurrentBalance=CurrentBalance+@amount where id=@AccountId";
        ...
        /// <param name="error"></param>
        /// <returns></returns>
        public static bool SubmitTransactionAndUnlock(ITransaction item, int lockId,out string error)
        {
            error = "";
            if (lockId <= 0)
            {
        ...
        /// 锁定一定金额
        /// </summary>
        public static bool LockAmount(ILockRecord record, out int id, out string message)
        {
            message = "";
            if (record.Amount <= 0)
            {
            ...
            DBExtend helper = dbHelper;
            string sql = "update $IAccountDetail set LockedAmount=LockedAmount+@LockedAmount where id=@AccountId and CurrentBalance-(LockedAmount+@LockedAmount)>=0";
            //sql = FormatTable(sql);
            helper.AddParam("LockedAmount", record.Amount);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static bool UnlockAmount(int id, out string message)
        {
            DBExtend helper = dbHelper;
            helper.BeginTran();
        ...
        /// <returns></returns>
        public static List<ITransaction> QueryAccountHistory(string account, int accountType, int transactionType, int pageIndex, int pageSize, out int count, DateTime? startTime, DateTime? endTime)
        {
            count = 0;
            ...
            int accountId = AccountAction<TType>.GetAccountId(account, accountType,transactionType);
            if (accountId == 0)
                return list;
            DBExtend helper = dbHelper;
